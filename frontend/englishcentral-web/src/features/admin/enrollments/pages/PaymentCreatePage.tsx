...
export function PaymentCreatePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([adminEnrollmentFinanceApi.getEnrollment(recordId), adminEnrollmentFinanceApi.getInvoices(recordId), adminMetadataApi.getPaymentMethodOptions()])
      .then(([nextEnrollment, invoiceResult, methodOptions]) => { setEnrollment(nextEnrollment); setInvoices(invoiceResult.items.filter((invoice) => invoice.outstandingAmount > 0)); setMethods(methodOptions); setMethod(methodOptions[0]?.value ?? ""); })
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const paymentPdf = await adminEnrollmentFinanceApi.createPayment({ studentId: enrollment.studentId, amount: total, method, referenceCode: referenceCode.trim() || null, payerName: payerName.trim() || null, payerPhone: payerPhone.trim() || null, notes: notes.trim() || null, allocations: selected });
  ...
  return <div className={`${formStyles.page} ${styles.page}`}><section className={formStyles.header}><div><Link className={formStyles.backLink} to={`/admin/enrollments/${recordId}/view`}><ArrowLeft size={16} /> Quay lại công nợ</Link><h1>Ghi nhận thanh toán</h1></div></section>
    {receiptPdfUrl ? <section className={styles.receipt}><h2>Thu tiền thành công</h2><p>BE đã trả về file PDF phiếu/hóa đơn thanh toán.</p><p><a href={receiptPdfUrl} rel="noreferrer" target="_blank">Mở file PDF</a></p><Link to={`/admin/enrollments/${recordId}/view`}>Xem lại công nợ</Link></section> :
